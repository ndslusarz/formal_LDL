...
Definition neg_impl_dl (R : realType) :=
  forall f1 f2 (e : @expr R (boolT_def impl_def f1 f2)),
    (`~ e) = (e `=> dl_bool neg_def impl_def f1 f2 false).

Definition true_false_dl (R : realType) :=
  forall f1 f2 f3,
    (@dl_bool R neg_def f1 f2 f3 true) = (`~ dl_bool neg_def f1 f2 f3 false).

Definition mand_impl_dl (R : realType) :=
  forall f (a b : @expr R (boolT_def impl_def m_def f)),
    a `** b = (`~ (a `=> `~ b)).
...
| bot_l : forall Q A b,
    seq_calc_luka_impl ((dl_bool neg_def _ _ _ false :: A |- [:: b]) :: Q)
|implL_l : forall Q A B a b,
    seq_calc_luka_impl ((b :: B |- a :: A) :: Q ) ->
    seq_calc_luka_impl ((((a `=> b) :: B) |- A) :: Q)
| implR_l : forall Q A B a b,
    seq_calc_luka_impl ((A |- B) :: Q ) ->
...
| andR_l : forall Q A B (a b : formula),
    ...
    seq_calc_luka_impl ((A |-  (a `/\ b) :: B) :: Q )
| orL_l : forall Q A B a b,
    seq_calc_luka_impl (((b :: A) |- B) :: Q) ->
    seq_calc_luka_impl (((a :: A) |- B) :: Q) ->
    seq_calc_luka_impl (((a `\/ b) :: A |- B) :: Q)
| orR_l : forall Q A B a b,
    seq_calc_luka_impl ((A |- a :: B ) :: ( A |- b :: B) :: Q ) ->
    seq_calc_luka_impl ((A |- (a `\/ b) :: B) :: Q) .

Local Hint Extern 0 (seq_calc_luka_impl ((_ |- _) :: _)) =>
  solve [apply: id_l] : core.
...
| bot_l' : forall Q A b,
    seq_calc_luka ((dl_bool neg_def _ _ _ false :: A |- [:: b]) :: Q)
| top_l' : forall Q A,
    seq_calc_luka ((A |- [:: (dl_bool neg_def _ _ _  true)]) :: Q)
| mandL_l' : forall Q A B a b,
    seq_calc_luka (((a :: b :: B) |- A) :: Q ) ->
    seq_calc_luka ((dl_bool neg_def _ _ _ false :: B |- A) :: Q ) ->
    seq_calc_luka ((((a `** b) :: B) |- A) :: Q)
| mandR_l' : forall Q A B a b,
    seq_calc_luka ((A |- B) :: Q ) -> (*not needed for soundness,
                                        but this is needed for this rule
                                        to be derivable*)
    seq_calc_luka ((A |- (a ::  b :: B)) :: (A |- dl_bool neg_def _ _ _ false :: B) :: Q)  ->
    seq_calc_luka ((A |- (a `** b) :: B) :: Q )
| negL_l' : forall Q A B a,
    seq_calc_luka (((dl_bool neg_def _ _ _ false :: A) |-  a :: B) :: Q) ->
    seq_calc_luka ((((`~ a) :: A) |- B) :: Q)
| negR_l' : forall Q A B a,
    seq_calc_luka ((A |- B):: Q) ->
    seq_calc_luka (((a :: A) |- dl_bool neg_def _ _ _ false :: B)::Q) ->
    seq_calc_luka ((A |- (`~a) :: B) :: Q)
| morL_l' : forall Q A B a b,
    seq_calc_luka ((A |- B) :: Q) ->
    seq_calc_luka (((a :: b :: A) |- dl_bool neg_def _ _ _ false :: B) :: Q) ->
    seq_calc_luka ((((a `++ b) :: A) |- B) :: Q)
| morR_l' : forall Q A B a b,
    seq_calc_luka ((A |- B):: Q) ->
    seq_calc_luka (((dl_bool neg_def _ _ _ false :: A) |- a :: b :: B) :: Q) ->
    seq_calc_luka ((A |- (a `++ b) :: B) :: Q)
| andL_l' : forall Q A B a b,
    seq_calc_luka (((a :: B) |- A) :: ((b :: B) |- A) :: Q) ->
    seq_calc_luka ((((a `/\ b) :: B) |- A) :: Q)
| andR_l' : forall Q A B a b,
    seq_calc_luka ((A |- a :: B) :: Q ) ->
...
Local Hint Extern 0 (seq_calc_luka ((_ |- _) :: _)) =>
  solve [apply: id_l'] : core.

Definition eval_luka A := 1%R + \sum_(i <- A) ([[i]]_Lukasiewicz - 1%R).

Lemma eval_luka_add_el A a :
...
rewrite subr_le0.
by have /andP[] := translate_boolT_01 p1 Lukasiewicz i.
Qed.
...
- move: IHseq_calc_luka_impl => [q1 + IH2].
  ...
  + move/eqP in h1.
    ...
    have helper :
      (eval_luka A + eval_luka B) - 1 <= (eval_luka C + eval_luka D) - 1 ->
      (eval_luka A + eval_luka B) - eval_luka D - 1 <= eval_luka C  - 1 .
      by intros; lra.
    apply helper in IH2.
    clear helper.
...
- move: IHseq_calc_luka_impl => [q + IH2].
  ...
  + by exists q; rewrite ?in_cons ?h ?orbT//=.
- exists (dl_bool neg_def  _ _ _ false :: A |- [:: b]); first by rewrite mem_head.
  rewrite //= !eval_luka_add_el addr0.
  have h := eval_luka1 A.
  have hb := translate_boolT_01 p1 Lukasiewicz b.
  have := hb => /andP[b0 b1].
  have helper : 0 <= (eval_luka [::] + [[b]]_Lukasiewicz) - 1 ->
                eval_luka A - 1 <= (eval_luka [::] + [[b]]_Lukasiewicz) - 1.
    by intros; lra.
...
- case IHseq_calc_luka_impl1 => [q1].
  ...
  + exists (A |- (a `=> b) :: B); first by rewrite mem_head.
    ...
    rewrite //= eval_luka_add_el//=/minr; case: ifPn => h.
    * have temp : (eval_luka A + [[a]]_Lukasiewicz) - 1 <= eval_luka B + [[b]]_Lukasiewicz - 1 ->
                  eval_luka A <= eval_luka B + (1 - [[a]]_Lukasiewicz + [[b]]_Lukasiewicz) - 1
        by intros; lra.
      by apply temp in IH22.
    * lra.
  + by exists q1 => //; rewrite in_cons h1 orbT.
  + by exists q2 => //; rewrite in_cons h2 orbT.
  + by exists q1 => //; rewrite in_cons h1 orbT.
...
- case IHseq_calc_luka_impl1 => [q1].
  ...
  + exists (A |- (a `/\ b) :: B); first by rewrite mem_head.
    ...
    have ha := translate_boolT_01 p1 Lukasiewicz a.
    have hb := translate_boolT_01 p1 Lukasiewicz b.
    by repeat case: ifPn; move => ? ?; lra.
  + by exists q1 => //; rewrite in_cons h1 orbT.
  + by exists q2 => //; rewrite in_cons h2 orbT.
  + by exists q1 => //; rewrite in_cons h1 orbT.
- case IHseq_calc_luka_impl1 => [q1].
...
- move: IHseq_calc_luka_impl => [q1 + IH2].
  ...
  + exists (A |- (a `\/ b) :: B); first by rewrite mem_head.
    subst; rewrite //= !eval_luka_add_el//=/maxR !big_ord_recl big_ord0 /maxr.
    rewrite //= !eval_luka_add_el in IH2.
    repeat case: ifPn; move=> h1 h2; lra.
  + exists (A |- (a `\/ b) :: B); first by rewrite mem_head.
    subst; rewrite //= !eval_luka_add_el//=/maxR !big_ord_recl big_ord0 /maxr.
    rewrite //= !eval_luka_add_el in IH2.
    by repeat case: ifP; move=> ? ?; lra.
  + by exists q1 => //; rewrite in_cons h3 orbT.
Qed.
...
- case: IHseq_calc_luka => [q + IH2].
  rewrite in_cons => /predU1P[h | h].
  + exists (A ++ C |- B); first by rewrite mem_head.
...
- exists (dl_bool neg_def _ _ _ false :: A |- [:: b]); first by rewrite mem_head.
  rewrite /= !eval_luka_add_el addr0.
  have h := eval_luka1 A.
  have /andP[b0 b1] := translate_boolT_01 p1 Lukasiewicz b.
  have : 0 <= eval_luka [::] + [[b]]_Lukasiewicz - 1 ->
         eval_luka A - 1 <= eval_luka [::] + [[b]]_Lukasiewicz - 1
    by intros; lra.
  by apply; rewrite /eval_luka//= big_nil addr0; lra.
- exists (A |- [:: dl_bool neg_def _ _ _ true]); first by rewrite mem_head.
...
- case IHseq_calc_luka1 => [q1].
  ...
  + subst.
    ...
    rewrite eval_luka_add_el.
    have /andP[ab0 ab1] := translate_boolT_01 p1 Lukasiewicz (a `/\ b).
    rewrite /= big_ord_recl !big_ord1 /maxr.
    case: ifPn => h_max.
    * by rewrite addr0; apply IH22.
    * rewrite addrA.
      have -> : eval_luka B + ([[a]]_Lukasiewicz + [[b]]_Lukasiewicz - 2) + 1 - 1 =
               (eval_luka B + [[b]]_Lukasiewicz - 1) + [[a]]_Lukasiewicz - 1 by lra.
      by rewrite IH12.
  + by exists q1 => //; rewrite in_cons h1 orbT.
  + by exists q2 => //; rewrite in_cons h2 orbT.
  + by exists q1 => //; rewrite in_cons h1 orbT.
- case: IHseq_calc_luka2 => [q + IH2].
  rewrite in_cons in_cons => /predU1P[h |/predU1P[h | h]].
  +  exists (A |- (a `** b) :: B); first by rewrite mem_head.
    * subst.
      rewrite //= !eval_luka_add_el in IH2.
      rewrite eval_luka_add_el.
      ...
      case: ifPn => h_max.
      + rewrite addr0.
        have hh : [[a]]_Lukasiewicz + [[b]]_Lukasiewicz - 2 + 1 =
                  [[a]]_Lukasiewicz + [[b]]_Lukasiewicz - 1 by lra.
        rewrite {}hh in h_max.
        by have ->// : eval_luka A <= eval_luka B + [[b]]_Lukasiewicz - 1 + [[a]]_Lukasiewicz - 1 ->
          [[a]]_Lukasiewicz + [[b]]_Lukasiewicz - 1 < 0 ->
          eval_luka A <= eval_luka B -1 by lra.
      + have -> :
          (eval_luka B + ((([[a]]_Lukasiewicz + [[b]]_Lukasiewicz) - 2) + 1))%R - 1 =
          (((eval_luka B + [[b]]_Lukasiewicz) - 1) + [[a]]_Lukasiewicz) - 1 by lra.
         by rewrite IH2.
    * exists (A |- (a `** b) :: B) ; first by rewrite mem_head.
      subst.
      ...
      have /andP[] := translate_boolT_01 p1 Lukasiewicz (a `** b).
      by lra.
  + by exists q=> //; rewrite in_cons h orbT.
- move: IHseq_calc_luka => [q + IH2].
  rewrite in_cons => /predU1P[h | h].
  + subst.
    exists ((`~ a) :: A |- B) => /=; first by rewrite mem_head.
    rewrite !eval_luka_add_el //= addr0 in IH2.
    by rewrite eval_luka_add_el/=; lra.
  + by exists q=> //; rewrite in_cons h orbT.
- case: IHseq_calc_luka2 => [q + IH2].
...
- move: IHseq_calc_luka2 => [q + IH2].
  ...
  + subst.
    ...
    case: ifPn => h_min.
    * have ->// :
        (eval_luka A + ([[a]]_Lukasiewicz + [[b]]_Lukasiewicz)) - 1 -1 <=
        eval_luka B -1 ->
        (eval_luka A + ([[a]]_Lukasiewicz + [[b]]_Lukasiewicz)) - 1 <=
        eval_luka B.
        by intros; lra.
      lra.
    * rewrite -leNgt in h_min.
      have helper : ((eval_luka A + [[b]]_Lukasiewicz) - 1) + [[a]]_Lukasiewicz - 1 <=
                    eval_luka B - 1 -> (eval_luka A) <= eval_luka B .
        by intros; lra.
  ...
  rewrite !in_cons //= => /predU1P[h2 | h2] IH22 /predU1P[h1 | h1] IH12.
   + subst.
     ...
     * have helper : eval_luka A - 1 <=
         (((eval_luka B + [[b]]_Lukasiewicz) - 1) + [[a]]_Lukasiewicz) - 1 ->
         eval_luka A <= (((eval_luka B + [[a]]_Lukasiewicz) + [[b]]_Lukasiewicz)) - 1.
         by intros; lra.
       apply helper in IH22.
       by rewrite addrA IH22.
     * lra.
  + by exists q1 => //; rewrite in_cons h1 orbT.
  + by exists q2 => //; rewrite in_cons h2 orbT.
  + by exists q1 => //; rewrite in_cons h1 orbT.
...
Lemma luka_neg_impl_admissable a :
 [[`~ a]]_Lukasiewicz = [[a `=> dl_bool _ _ _ _ false]]_Lukasiewicz.
Proof.
rewrite /= addr0 /minr; case: ifPn; intros; first by [].
have h := translate_boolT_01 p1 Lukasiewicz a.
rewrite -leNgt in n.
...
Lemma luka_true_false_admissable :
  [[@dl_bool R neg_def impl_def m_def l_def true]]_Lukasiewicz =
  [[`~ dl_bool neg_def impl_def m_def l_def false]]_Lukasiewicz.
Proof. by rewrite//= subr0. Qed.

Lemma luka_mand_impl_admissable a b :
  [[a `** b]]_Lukasiewicz = [[`~ (a `=> `~b)]]_Lukasiewicz.
Proof.
rewrite /= /maxr/minr.
case: ifPn; case: ifPn => //= h1 h2; try lra.
- rewrite !big_ord_recl big_ord0 addr0 in h2.
  have ha := translate_boolT_01 p1 Lukasiewicz a.
  have hb := translate_boolT_01 p1 Lukasiewicz b.
  have helper1 : ((1 - [[a]]_Lukasiewicz) + (1 - [[b]]_Lukasiewicz)) < 1 ->
                 [[a]]_Lukasiewicz + [[b]]_Lukasiewicz > 1.
    by intros; lra.
  apply helper1 in h1.
  have helper2 : ((([[a]]_Lukasiewicz + [[b]]_Lukasiewicz) - 2) + 1) < 0 ->
      [[a]]_Lukasiewicz + [[b]]_Lukasiewicz < 1.
    by intros; lra.
  apply helper2 in h2.
  lra.
- rewrite !big_ord_recl big_ord0 addr0 in h2.
  ...
  have -> : 1 - ((1 - [[a]]_Lukasiewicz) + (1 - [[b]]_Lukasiewicz)) =
                  1 - ((2 - [[a]]_Lukasiewicz - [[b]]_Lukasiewicz)) by lra.
  have -> : 1 - (2 - [[a]]_Lukasiewicz - [[b]]_Lukasiewicz) =
                   -1 + [[a]]_Lukasiewicz + [[b]]_Lukasiewicz by lra.
  have -> : ((([[a]]_Lukasiewicz + [[b]]_Lukasiewicz) - 2) + 1) =
                   ([[a]]_Lukasiewicz + [[b]]_Lukasiewicz) - 1 by lra.
  lra.
- rewrite !big_ord_recl big_ord0 addr0 in h2.
  rewrite !big_ord_recl big_ord0 addr0//=.
  have helper3 : [[a]]_Lukasiewicz + [[b]]_Lukasiewicz - 2 + 1 =
    [[a]]_Lukasiewicz + [[b]]_Lukasiewicz - 1 by lra.
  rewrite helper3//=. rewrite helper3 in h2.
  rewrite -!leNgt in h2, h1.
  lra.
Qed.

Lemma luka_mor_impl_admissable a b :
  [[a `++ b]]_Lukasiewicz = [[(`~ a) `=> b]]_Lukasiewicz.
Proof.
rewrite /= /maxr /minr.
have helper : (1 - (1 - [[a]]_Lukasiewicz)) + [[b]]_Lukasiewicz =
...
- rewrite helper.
  by apply/eqP; rewrite eq_le !leNgt h2/=; lra.
Qed.

(*specific exchange rules for simpler cases*)
Lemma luka_cat1C Q (I : seq _ * seq _) :
  seq_calc_luka_impl (I :: Q) = seq_calc_luka_impl ([:: I] ++ Q).
Proof. by []. Qed.

Lemma luka_catC Q P :
  seq_calc_luka_impl (P ++ Q) -> seq_calc_luka_impl (Q ++ P).
Proof.
move=> H.
...
Lemma luka_implL_extended Q A B a b:
  seq_calc_luka_impl ((B |- A) :: (b :: B |- a :: A) :: Q ) ->
  seq_calc_luka_impl (((a `=> b) :: B |- A) :: Q).
Proof.
move => H; rewrite luka_cat1C.
apply/luka_catC/ec_l.
rewrite -(cat1s (a `=> b) B).
apply/luka_catC/luka_exL_nil/w_l.
rewrite -(cat1s _ Q) -cat1s.
apply luka_catC.
rewrite -catA.
apply (@luka_catC _ (Q ++ [:: B |- A])).
apply/luka_catC/implL_l.
rewrite -cat1s.
move: H.
...
Lemma equivalence_luka Q : seq_calc_luka Q -> seq_calc_luka_impl Q.
Proof.
move=> H; dependent induction H.
- by [].
- exact/eex_l/IHseq_calc_luka.
...
- rewrite mand_impl_luka neg_impl_luka.
  ...
  + rewrite neg_impl_luka. apply implR_l.
    * have -> : [:: a, dl_bool _ _ _ _ false & B] =
                [:: a] ++ (dl_bool _ _ _ _ false :: B) by [].
      apply/luka_exL_nil/w_l.
      by exact: IHseq_calc_luka2.
    * rewrite (_ : [:: b, a, _ & B] = [:: b; a] ++ dl_bool _ _ _ _ false :: B)//.
      apply luka_exL_nil.
      have -> : (dl_bool _ _ _ _ false :: B) ++ [:: b; a] =
                [::dl_bool _ _ _ _ false] ++ B ++ [:: b; a] by [].
      rewrite -(cat1s _ A).
      apply: mix_l.
      - by [].
      - have <- : [:: b] ++ [:: a] ++ [::] = [:: b; a] by [].
...
- rewrite mand_impl_luka neg_impl_luka.
  ...
  + apply luka_implL_extended. rewrite neg_impl_luka.
    have -> : [:: A |- dl_bool _ _ _ _ false :: B,
                     (b `=> dl_bool _ _ _ _ false) :: A |- [:: a, dl_bool _ _ _ _ false & B] & Q] =
                    [:: A |- dl_bool _ _ _ _ false :: B] ++ ((
                       (b `=> dl_bool _ _ _ _ false) :: A |- [:: a, dl_bool _ _ _ _ false & B]) :: Q).
      by [].
    apply luka_catC => /=.
    apply luka_implL_extended.
    have -> : [:: A |- [:: a, dl_bool _ _ _ _ false & B],
                      dl_bool _ _ _ _ false :: A |- [:: b, a, dl_bool _ _ _ _ false & B]
      & Q ++ [:: A |- dl_bool _ _ _ _ false :: B]] =
                     [:: A |- [:: a, dl_bool _ _ _ _ false & B]] ++
                       ((dl_bool _ _ _ _ false :: A |- [:: b, a, dl_bool _ _ _ _ false & B]) ::
      Q ++ [:: A |- dl_bool _ _ _ _ false :: B]).
      by [].
    apply luka_catC.
    apply ew_l .
    have -> : dl_bool _ _ _ _ false :: A |- [:: b, a, dl_bool _ _ _ _ false & B] =
                     ([::dl_bool _ _ _ _ false] ++ A |- [:: b] ++ [:: a] ++ [:: dl_bool _ _ _ _ false] ++ B).
      by [].
    apply exR_l. apply(@luka_exR_nil _ ([:: dl_bool _ _ _ _ false] ++ [:: a] ++ B) ([:: b])).
    have -> : ([:: dl_bool  _ _ _ _ false] ++ [:: a] ++ B) ++ [:: b] =
              ([:: dl_bool  _ _ _ _ false]) ++ ([:: a] ++ B ++ [:: b]) by [].
    apply: mix_l.
    * by [].
    * rewrite catA. apply luka_exR_nil.
      have -> : [:: b] ++ [:: a] ++ B = [::] ++ [:: b] ++ [:: a] ++ B by [].
      apply exR_l. rewrite//=.
      have -> : (A |- [:: a, b & B]) :: Q ++ [:: A |- dl_bool  _ _ _ _ false :: B] =
                ([::A |- [:: a, b & B]] ++ Q ++ [:: A |- dl_bool  _ _ _ _ false :: B] ++ [::]) by [].
...
- rewrite mor_impl_luka. apply luka_implL_extended.
  ...
  have -> : [:: A |- B, b :: A |- (a `=> dl_bool  _ _ _ _ false) :: B & Q] =
            [:: A |- B] ++ ((b :: A |- (a `=> dl_bool  _ _ _ _ false) :: B) :: Q) by [].
  apply: luka_catC => /=.
  apply: implR_l.
  + rewrite -(cat1s b A).
    apply/luka_exL_nil/w_l.
    ...
    rewrite -(cat1s (A |- B)).
    exact: IHseq_calc_luka1.
  + have -> : ([:: a, b & A] |- dl_bool  _ _ _ _ false :: B) :: Q ++ [:: A |- B] =
              (([:: a, b & A] |- dl_bool  _ _ _ _ false :: B) :: Q) ++ [:: A |- B] by [].
    apply ew_l.
    exact: IHseq_calc_luka2.
- rewrite mor_impl_luka. apply implR_l.
  ...
  + rewrite neg_impl_luka. apply implL_l.
    exact: IHseq_calc_luka2.
- apply andL_l. exact IHseq_calc_luka.
- apply andR_l.
  + exact IHseq_calc_luka1.
  + exact IHseq_calc_luka2.
- apply orL_l.
  + exact IHseq_calc_luka1.
  + exact IHseq_calc_luka2.
- apply orR_l. exact IHseq_calc_luka.
Qed.
...
| bot_p : forall Q A B (b : formula),
    seq_calc_product ((dl_bool _ _ _ _ false :: A |- B) :: Q)
...
| bot_p' : forall Q A B (b : formula),
    seq_calc_product' ((dl_bool _ _ _ _ false :: A |- B) :: Q)
| top_p' : forall Q A,
   seq_calc_product' ((A |- [:: dl_bool _ _ _ _ true]) :: Q)
| mandL_p' : forall Q A B (a b : formula),
    seq_calc_product' (((a :: b :: B) |- A) :: Q ) ->
    seq_calc_product' ((((a `** b) :: B) |- A) :: Q )
| mandR_p' : forall Q A B (a b : formula),
...
| negR_p' : forall Q A B (a : formula),
    seq_calc_product' ((A |- B) :: Q) ->
    seq_calc_product' ((a :: A |- dl_bool _ _ _ _ false :: B) :: Q) ->
    seq_calc_product' ((A |- (`~ a) :: B) :: Q)
| negL_p' : forall Q A B (a : formula),
    seq_calc_product' ((B |- [::a]) :: Q) ->
    seq_calc_product' ((((`~a) :: B) |- A) :: Q)
...
- have /andP[? ?] := translate_boolT_01 p1 product a.
...
- case IHseq_calc_product1 => [q1].
  ...
  rewrite !in_cons //= => /predU1P[h2 | h2] IH12 /predU1P[h1 | h1] IH22.
  + exists (A ++ B |- C ++ D); subst; first by rewrite mem_head.
    rewrite //= in IH12 IH22.
...
- case: IHseq_calc_product => [q1 + IH2].
  ...
  + by exists q1 => //; rewrite !in_cons h2 !orbT.
- exists (dl_bool _ _ _ _ false :: A |- B).
  + by rewrite mem_head.
...
- case: IHseq_calc_product => [q1 + IH2].
  rewrite in_cons => /predU1P[h1|h2].
  + subst.
    exists ((`~ a) :: B |- A); first by rewrite mem_head.
    ...
    * rewrite mul1r.
      have := translate_boolT_01 p1 product a.
...
- case IHseq_calc_product1 => [q1].
  ...
  + subst q1 q2.
    ...
    case: ifPn => [ba|ba].
    * have ha := translate_boolT_01 p1 product a.
      have hb := translate_boolT_01 p1 product b.
...
- case IHseq_calc_product'1 => [q1].
  ...
  + exists (A ++ B |- C ++ D); subst; first by rewrite mem_head.
    rewrite /= !eval_product_add.
    rewrite /= in IH12 IH22.
    have le_mul : forall a b c : R, 0 <= a -> 0 <= b -> a <= c -> b <= 1 ->
      a * b <= c * b by intros; nra.
...
- move: IHseq_calc_product' => [q1 + IH2].
  ...
  + subst.
    ...
    * have ha := translate_boolT_01 p1 product a.
      have hb := translate_boolT_01 p1 product b.
      have hB := eval_product_01 B. rewrite /eval_product in hB.
      move/negbT : h2; rewrite -leNgt => h2.
      have hA' : [[b]]_product * \prod_(j <- B) [[j]]_product <= \prod_(i <- A) [[i]]_product
        by intros; nra.
      exact hA'.
    * rewrite mul1r.
      have ha := translate_boolT_01 p1 product a.
      move/negbT : h2; rewrite -leNgt /tnth => /= h2.
      move/negbT : h1; rewrite -leNgt /tnth => /= h1.
      move: IH2.
      have -> : [[a]]_product = 1 by lra.
      by rewrite mul1r.
  + subst.
    exists ((a `/\ b) :: B |- A); first by rewrite mem_head.
    rewrite //= /eval_product !big_cons//= /minR !big_ord_recl big_ord0 /minr.
    rewrite//= /eval_product !big_cons in IH2.
    repeat case: ifPn; move=> h1 h2; try nra.
    * have hB := eval_product_01 B. rewrite /eval_product in hB.
      have h' : [[a]]_product * \prod_(j <- B) [[j]]_product <= \prod_(i <- A) [[i]]_product
        by intros; nra.
      by exact h'.
    * have /andP[_ b1] := translate_boolT_01 p1 product b.
      rewrite /tnth /= -leNgt in h1.
      have {}b1 : [[b]]_product = 1 by apply/eqP; rewrite eq_le h1 andbT.
      move: IH2; rewrite b1 mul1r /tnth/= => IH2.
      rewrite /tnth/= in h2.
      rewrite (le_trans _ IH2)// ler_piMl//.
        by have /andP[] := eval_product_01 B.
      exact: ltW.
    * rewrite mul1r.
      have hb := translate_boolT_01 p1 product b.
...
Lemma product_true_false_admissable :
  [[@dl_bool R neg_def impl_def m_def l_def true]]_product =
    [[`~ dl_bool neg_def impl_def m_def l_def false]]_product.
...
- apply mix_p.
  + exact IHseq_calc_product'1.
  + exact IHseq_calc_product'2.
...
- apply orL_p.
  + exact: IHseq_calc_product'1.
  + exact: IHseq_calc_product'2.
- apply orR_p. exact: IHseq_calc_product'.
Qed.
...
(*logical*)
| bot_g : forall Q A B,
    seq_calc_godel ((dl_bool _ _ _ _ false :: A |- B) :: Q)
| top_g : forall Q A,
    seq_calc_godel ((A |- [::dl_bool _ _ _ _ true]) :: Q )
...
| negR_g : forall Q A (a : formula),
    seq_calc_godel' ((a :: A |- [:: dl_bool _ _ _ _ false]) :: Q) ->
    seq_calc_godel' ((A |- [:: (`~ a)]) :: Q)
| negL_g : forall Q A1 A2 B (a : formula),
    seq_calc_godel' ((A1 |- [:: a]) :: Q) ->
    seq_calc_godel' ((dl_bool _ _ _ _ false :: A2 |- B) :: Q) ->
    seq_calc_godel' (((`~ a) :: A1 ++ A2 |- B) :: Q).

Lemma big_maxr_godel_le1 n (A : 'I_n -> formula) :
   \big[maxr/0]_(j < n) [[A j]]_Godel <= 1.
Proof.
have := translate_boolT_01 p1 Godel (dl_or A).
by rewrite /= /maxR; case/andP.
Qed.

Lemma big_minr_godel_le1 n (A : 'I_n -> formula) :
  \big[minr/1]_(j < n) [[A j]]_Godel <= 1.
Proof.
have := translate_boolT_01 p1 Godel (dl_and A).
by rewrite /= /minR; case/andP.
Qed.
...
- case IHseq_calc_godel => [M + IH2].
  by rewrite !mem_cat => /orP[h|/orP[h|h]]; exists M => //; rewrite mem_cat h// orbT.
...
- move: IHseq_calc_godel => [q + IH2].
  ...
  + exists ((a `/\ b) :: B |- A); subst; first by rewrite mem_head.
    ...
    have hb : minr ([[b]]_Godel) 1 = [[b]]_Godel.
      apply/min_idPl.
      by have /andP[] := translate_boolT_01 p1 Godel b.
  ...
  + move/predU1P : IH1 => [|IH1].
    ...
    have hb : minr ([[b]]_Godel) 1 = [[b]]_Godel.
      have h := translate_boolT_01 p1 Godel b.
...
- case IHseq_calc_godel1 => [q1].
  ...
  + subst.
    ...
    have hb_max (x : @expr R (boolT_def impl_def m_def l_def)) :
      ...
      have h := translate_boolT_01 p1 Godel x.
...
- case IHseq_calc_godel'1 => [q1].
  case IHseq_calc_godel'2 => [q2].
  rewrite !in_cons //= => /predU1P[h2 | h2] + /predU1P[h1 | h1].
  + subst.
    rewrite /= !big_cons !big_nil.
    have hb_max (x : @expr R (boolT_def impl_def m_def l_def)) :
        (maxr ([[x]]_Godel) 0) = [[x]]_Godel.
...
- case: IHseq_calc_godel' => [q + IH2].
  ...
  + exists (A |- [:: `~ a]); subst; first by rewrite mem_head.
    ...
    * rewrite /maxr; case: ifPn; intros; rewrite/=;
      ...
      - by lra.
      - have ha := translate_boolT_01 p1 Godel a.
        have := translate_boolT_01 p1 Godel (dl_and (tnth (in_tuple A))).
        by lra.
...
- case IHseq_calc_godel'1 => [q1].
  case IHseq_calc_godel'2 => [q2].
  rewrite !in_cons //= => /predU1P [h2 | h2] IH22 /predU1P[h1 | h1] IH12.
  + exists ((`~ a) :: A1 ++ A2 |- B); subst; first by rewrite mem_head.
    ...
    * rewrite /maxr in IH12. move: IH12.
      ...
      have hA1 := translate_boolT_01 p1 Godel (dl_and (tnth (in_tuple A1))).
      rewrite//= /minR in hA1.
      ...
      have := big_minr_if A1 A2.
      have hA2 := translate_boolT_01 p1 Godel (dl_and (tnth (in_tuple A2))).
      rewrite//= /minR in hA2.
      case: ifPn; intros; rewrite//=.
